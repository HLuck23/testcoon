...
-- ============================================================
-- PED HEADSHOT CACHE
-- Maps server source id → txd string (e.g. "pedheadshot_txd_0")
-- Headshots are registered once per player and reused until they
-- leave. The txd string is sent to the NUI so it can render the
-- operator portrait in the Siege-style top-bar team UI.
-- ============================================================
local HeadshotCache = {}   -- [serverId] = { txd = "...", handle = N }

-- Generation counter per player — incremented on every new capture.
-- The background polling thread only writes back if gen still matches,
-- preventing a stale thread from overwriting a newer capture.
local HeadshotGen = {}

local function CaptureHeadshot(serverId, playerId)
    if HeadshotCache[serverId] and HeadshotCache[serverId].txd then
        return
    end
    ...
    -- Clean up any in-progress handle (this resource's own internal one,
    -- if present -- callers like the round-robin loop and the manual
    -- outfit-change hook already call ReleaseHeadshot() externally before
    -- calling this function, which is the unregister that actually matters
    -- for the fix below)
    if HeadshotCache[serverId] and HeadshotCache[serverId].handle then
        pcall(UnregisterPedheadshot, HeadshotCache[serverId].handle)
    end
    HeadshotCache[serverId] = nil

    -- FIX (the "choosing between two" / alternating stale-slot bug): every
    -- caller of this function (the round-robin loop, the manual outfit-
    -- change hook) does ReleaseHeadshot() immediately followed by
    -- CaptureHeadshot() with ZERO yield in between -- Unregister and
    -- Register happen in the exact same game tick. With only ~2-3 internal
    -- headshot render slots in rotation, calling RegisterPedheadshot again
    -- that fast for the same ped can hand back a handle pointing at a slot
    -- the engine hasn't actually finished resetting yet -- it reports
    -- ready=true and returns a valid txd string quickly (confirmed via
    -- debug logging: ~170ms, no errors), but the ACTUAL PIXELS in that
    -- slot's render target are still whatever was there from the last time
    -- that exact slot was populated, not a fresh render of the ped's
    -- current appearance. That's why it was "100% predictable" and tied to
    -- which slot index came up -- one slot was reliably stale, the other
    -- reliably fresh, regardless of when the capture actually ran.
    --
    -- This always waits a single frame before registering, unconditionally
    -- -- not just when THIS function's own internal cleanup ran -- because
    -- the unregister that actually matters happens in the CALLER, via a
    -- separate ReleaseHeadshot() call, a statement earlier. By the time we
    -- get here HeadshotCache[serverId] is already nil either way, so there
    -- was no reliable way to detect "did an unregister just happen" from
    -- inside this function -- the only safe fix is to always yield one
    -- frame before RegisterPedheadshot, regardless of whether a prior
    -- handle existed. For first-time captures (nothing to unregister) this
    -- just adds 100ms of latency, imperceptible for a portrait that's about
    -- to take another ~170ms to actually render anyway.
    Citizen.CreateThread(function()
        Citizen.Wait(100)

        local handle = RegisterPedheadshot(targetPed)
        if not IsPedheadshotValid(handle) or handle == 0 then
            handle = RegisterPedheadshotTransparent(targetPed)
            if not IsPedheadshotValid(handle) or handle == 0 then
                return
            end
        end

        -- Bump generation so any previous polling thread knows it's stale
        HeadshotGen[serverId] = (HeadshotGen[serverId] or 0) + 1
        local myGen = HeadshotGen[serverId]
        local startTime = GetGameTimer()

        HeadshotCache[serverId] = { handle = handle, txd = nil }
        ...
        -- Bail if a newer capture started after us
        if HeadshotGen[serverId] ~= myGen then
            return
        end
        if not IsPedheadshotValid(handle) then
            if HeadshotGen[serverId] == myGen then HeadshotCache[serverId] = nil end
            return
        end
        ...
        if type(txd) ~= 'string' or txd:gsub('%s','') == '' then
            pcall(UnregisterPedheadshot, handle)
            if HeadshotGen[serverId] == myGen then HeadshotCache[serverId] = nil end
            return
        end

        if HeadshotGen[serverId] ~= myGen then return end

        if HeadshotCache[serverId] then
            HeadshotCache[serverId].txd = txd
        end
        SendNUIMessage({ type = "headshotReady", id = serverId, txd = txd })
    end)
end

local function ReleaseHeadshot(serverId)
    if HeadshotCache[serverId] then
        if HeadshotCache[serverId].handle then
            pcall(UnregisterPedheadshot, HeadshotCache[serverId].handle)
        end
        HeadshotCache[serverId] = nil
    end
...
-- Attach cached txd strings to a roster list before sending to NUI
local function EnrichRosterWithHeadshots(list)
    local enriched = {}
    for _, p in ipairs(list) do
        local entry = {}
        for k, v in pairs(p) do entry[k] = v end
        local cached = HeadshotCache[p.id]
        if cached and cached.txd then
            entry.portrait = cached.txd
        end
...
local NPC_POS       = vector4(-2975.6699, -1454.7476, 741.2398, 96.3372)
...
-- FIX: same mapping as the JS side's teamDisplayName -- internal team
-- keys stay "alpha"/"bravo" (server SESSION.teams.alpha/.bravo, color
-- tables, etc.), this is just the one place that converts them to the
-- player-facing "Team 1"/"Team 2" labels instead of calling :upper()
-- directly on the raw key everywhere a message gets built.
local TEAM_DISPLAY_NAME = { alpha = "Team 1", bravo = "Team 2" }
local function TeamDisplayName(team)
    return TEAM_DISPLAY_NAME[team] or (team and team:upper() or "?")
end
    ...
    while true do
        ...
        if hubState == "hub" and not IsInWars and not IsWaitingWars then
            ...
            if dist <= PROMPT_DIST then
                ...
                Draw3DTextWars(npcCoords.x, npcCoords.y, npcCoords.z + 1.3, "Wars", 0.55, alpha, 248, 250, 252)

                if dist <= INTERACT_DIST then
                    Draw3DTextWars(npcCoords.x, npcCoords.y, npcCoords.z + 1.05, "[E]  Choose a Map", 0.45, alpha, 160, 60, 255)
...
AddEventHandler('pvp-wars:leaveWaitingConfirmed', function()
    ...
    -- FIX: matches pvp-turf's "clearEquipped" pattern -- reset the
    -- shop/inventory's equipped-weapon display the instant we leave,
    -- same moment the server clears SESSION.loadouts[src], so nothing
    -- stale can ever be shown on the next visit.
    SendNUIMessage({ type = "realEquippedWeapon", weapon = nil })
...
-- ============================================================
-- LOADOUT SHOP  —  [F4].
-- REAL FIX (confirmed by diffing pvp-turf's working arsenal menu):
-- the close-not-working bug had nothing to do with IsShopOpen state,
-- and nothing to do with sharing F4 with pvp-turf. The actual cause:
-- once SetNuiFocus(true, true) is active, FXServer's NUI layer
-- captures ALL keyboard input -- RegisterKeyMapping/RegisterCommand
-- NEVER receives the second F4 press at all while the panel is
-- focused, so Lua has no way to "miss" it, it simply never arrives.
-- That's exactly why Escape and mouse-clicks (handled directly by
-- this page's own JS, see index.html) kept working while F4 (which
-- only existed as a Lua RegisterCommand) didn't. pvp-turf's arsenal
-- menu has a window.addEventListener("keydown", ...) inside its own
-- NUI page that catches F4 directly while focused and closes itself
-- -- added the exact same listener to pvp-wars's index.html. F4 is
-- back as the single key, no separate key needed.
-- ============================================================

RegisterKeyMapping('warsshop', 'PvP Wars: Loadout Shop', 'keyboard', 'F4')
...
AddEventHandler('pvp-wars:teamAssigned', function(side)
    ...
    Notify(("You've been assigned to %s. Waiting for session to start..."):format(TeamDisplayName(side)), {r, g, b})
...
function TeleportToWars(spawnPos, team)
    ...
    if team then
        ...
        Notify(("Deployed to %s — You are on %s"):format(WarsState.mapName or "?", TeamDisplayName(team)), {r, g, b})
    else
...
AddEventHandler('pvp-wars:rosterSync', function(alphaList, bravoList)
    ...
    -- Capture headshots for players not yet cached.
    local activePlayers = GetActivePlayers()
    for _, localId in ipairs(activePlayers) do
        local sid = GetPlayerServerId(localId)
        if not HeadshotCache[sid] or not HeadshotCache[sid].txd then
            CaptureHeadshot(sid, localId)
        end
    end

    -- Enrich each roster entry with the cached txd portrait string
    -- (nil for players whose headshot isn't ready yet -- the NUI
    -- swaps it in when it receives 'headshotReady' a moment later).
    local enrichedAlpha = EnrichRosterWithHeadshots(alphaList)
    local enrichedBravo = EnrichRosterWithHeadshots(bravoList)

    SendNUIMessage({
        type   = "rosterSync",
        alpha  = enrichedAlpha,
        bravo  = enrichedBravo,
        myTeam = WarsState.team,
    })
end)

-- NEW: a single player's alive/dead status flipped (death detected,
-- or respawn approved) -- updates just that one row in the team UI
-- on everyone's screen without needing a full roster resync.
RegisterNetEvent('pvp-wars:aliveStatusChanged')
AddEventHandler('pvp-wars:aliveStatusChanged', function(src, isAlive)
    SendNUIMessage({ type = "aliveStatusChanged", id = src, alive = isAlive })
end)
                ...
                if playerId ~= PlayerId() then
                    ...
                    if DoesEntityExist(targetPed) and not IsEntityDead(targetPed) then
                        ...
                        if markerInfo and dist < 60.0 then
                            ...
                            local r, g, b    = GetTeamColor(team)
                            -- (both branches of the old ternary here produced the identical
                            -- string regardless of team == myTeam -- simplified to one line)
                            local label      = ("[" .. TeamDisplayName(team) .. "]  " .. markerInfo.name)
...
AddEventHandler('gameEventTriggered', function(eventName, data)
    ...
    -- NEW: tell the server immediately so teammates' (and the team
    -- UI's) alive/dead indicator updates live, same moment death is
    -- detected -- not after the 3s respawn delay below.
    TriggerServerEvent('pvp-wars:reportDeathStatus')
...
AddEventHandler('pvp-wars:sessionEnd', function(reason)
    ...
    -- Release all cached headshot handles
    for sid, _ in pairs(HeadshotCache) do ReleaseHeadshot(sid) end
    HeadshotCache = {}

    SendNUIMessage({ type = "sessionEnd", reason = reason })
    SetNuiFocus(false, false)
    -- FIX: same clearEquipped-style reset as the leave handlers.
    SendNUIMessage({ type = "realEquippedWeapon", weapon = nil })
...
AddEventHandler('pvp-wars:leaveConfirmed', function()
    ...
    TeamMarkers = {}
    -- FIX: same clearEquipped-style reset as leaveWaitingConfirmed.
    SendNUIMessage({ type = "realEquippedWeapon", weapon = nil })
...
AddEventHandler('pvp-wars:adminFeedback', function(msg, success)
    -- FIX: this used to ALSO call Notify(msg, color), which posted the
    -- exact same message to chat as "[WARS] Map set to X." etc. -- pure
    -- clutter, since the admin panel's own feedback box (sent right
    -- below, unchanged) already shows this while the panel is open,
    -- which is the only time this event ever fires. Chat is no longer
    -- touched for admin actions at all.
...
-- ============================================================
-- HEALTH SYNC THREAD
-- Periodically sends the local player's current HP to the server
-- so the team UI (siege bar) on every teammate's screen stays live.
-- Runs every 1s while in an active or waiting Wars session.
-- ============================================================
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsInWars or IsWaitingWars then
            local ped = PlayerPedId()
            -- GetEntityHealth: 100 = empty, 200 = full. Convert to 0-100%.
            local raw = GetEntityHealth(ped)
            local hp  = math.max(0, math.min(100, raw - 100))
            if IsEntityDead(ped) then hp = 0 end
            TriggerServerEvent('pvp-wars:healthUpdate', hp)
        end
    end
end)
...
-- PedMngr broadcasts 'pedmngr:playerAppearanceUpdated' to ALL
-- clients the moment a player saves a new outfit. We catch it
-- here and immediately re-capture that player's headshot so the
-- team UI portrait updates in ~1 second instead of waiting for
-- the round-robin loop (which is 6s x number of active players).
--
-- The 600ms SetTimeout before re-capturing gives GTA's network
-- ped-component sync time to propagate the new components to
-- this client first -- without it we'd snapshot the ped while
-- it still has the OLD outfit on, producing the same stale
-- portrait we were trying to fix.
-- ============================================================

-- Tracks the last time we manually force-refreshed a player's headshot
-- via the appearance-change hook. The round-robin loop below checks this
-- so it doesn't fire a SECOND, redundant Release+Capture for the same
-- player a moment later -- two competing recaptures for the same ped in
-- quick succession is unnecessary and was adding to the instability.
local recentlyForcedRefresh = {}

RegisterNetEvent('pedmngr:playerAppearanceUpdated')
AddEventHandler('pedmngr:playerAppearanceUpdated', function(changedSrc)
    if not (IsInWars or IsWaitingWars) then
        return
    end

    -- FIX: this guard used to be set INSIDE the 600ms delayed callback below,
    -- which left a ~600ms window right after every outfit change where the
    -- round-robin loop could still land on this same player and fire its own
    -- competing Release+Capture -- two recaptures racing on the same ped,
    -- whichever's poll thread finishes can win regardless of which one
    -- actually captured the correct (post-sync) appearance. Setting it the
    -- instant the event arrives closes that window almost entirely -- the
    -- round-robin loop would need to land on this exact player in the same
    -- tick the event is received, which is effectively never.
    local sidNow = changedSrc
    recentlyForcedRefresh[sidNow] = GetGameTimer()
    ...
    Citizen.SetTimeout(600, function()
        ...
        for _, localId in ipairs(activePlayers) do
            local sid = GetPlayerServerId(localId)
            if sid == changedSrc then
                found = true
                recentlyForcedRefresh[sid] = GetGameTimer()
                ReleaseHeadshot(sid)
                CaptureHeadshot(sid, localId)
                break
            end
...
-- ============================================================
-- PORTRAIT REFRESH LOOP
-- Every 6s, pick the next player in rotation and re-capture
-- their headshot. Uses generation counter so no thread races.
-- ============================================================

local refreshRoundRobin = 1

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(6000)
        if not (IsInWars or IsWaitingWars) then goto cont end

        local players = GetActivePlayers()
        if #players == 0 then goto cont end

        if refreshRoundRobin > #players then refreshRoundRobin = 1 end
        local localId = players[refreshRoundRobin]
        refreshRoundRobin = refreshRoundRobin + 1

        local sid = GetPlayerServerId(localId)
        local ped = GetPlayerPed(localId)
        if not DoesEntityExist(ped) then goto cont end

        -- Skip if this player's headshot was manually force-refreshed
        -- (outfit change) in roughly the last 10 seconds. Avoids this
        -- loop stomping on top of a recapture that's still settling.
        local lastForced = recentlyForcedRefresh[sid]
        if lastForced and (GetGameTimer() - lastForced) < 10000 then
            goto cont
        end

        -- Clear cache so CaptureHeadshot will run (generation bump handles thread safety)
        ReleaseHeadshot(sid)
        CaptureHeadshot(sid, localId)
