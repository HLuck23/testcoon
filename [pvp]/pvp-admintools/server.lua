...
--
-- WARADMIN (Wars-only sub-admin role)
-- pvp-wars now supports a separate "waradmin" ACE for players who
-- should only be able to manage Wars sessions (assign teams, start/
-- end sessions, open the Wars admin panel) WITHOUT global pvpadmin
-- access to economy, stats, weapon resets, etc.
-- Grant it in server.cfg (replace with real identifiers/role IDs):
--   add_ace identifier.discord:PLACEHOLDER_DISCORD_ID waradmin allow
-- Or by group:
--   add_principal identifier.discord:PLACEHOLDER_DISCORD_ID group.waradmin
--   add_ace group.waradmin waradmin allow
-- ============================================================
