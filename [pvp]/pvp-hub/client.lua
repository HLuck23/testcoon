    ...
    while true do
        ...
        if HUB.inHub and HUB.npcEntity and DoesEntityExist(HUB.npcEntity) then
            ...
            if dist <= HUB.promptDist then
                ...
                -- NPC Name: bigger, white, higher
                Draw3DText(npcPos.x, npcPos.y, npcPos.z + 1.25, HUB.npcName, 0.55, alpha)

                if dist <= HUB.interactDist then
                    -- Prompt: bigger, white, higher
                    Draw3DText(npcPos.x, npcPos.y, npcPos.z + 1.05, "[E]  Enter Redzone", 0.45, alpha)
...
function Draw3DText(x, y, z, text, scale, alpha)
