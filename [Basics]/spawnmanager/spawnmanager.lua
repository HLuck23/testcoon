    ...
    add('spawnpoint', function(state, model)
        ...
        return function(opts)
            ...
            local s, e = pcall(function()
                ...
                -- get a heading and force it to a float, or just default to 272.2968
                heading = opts.heading and (opts.heading + 0.01) or 272.2968
...
function spawnPlayer(spawnIdx, cb)
    ...
    Citizen.CreateThread(function()
        ...
        if type(spawnIdx) == 'table' then
            ...
            spawn.heading = spawn.heading and (spawn.heading + 0.00) or 272.2968
        else
        ...
        if spawn.model then
            ...
            -- load the model for this spawn (max 10s timeout)
            local modelTimer = GetGameTimer()
            while not HasModelLoaded(spawn.model) do
                RequestModel(spawn.model)
                if (GetGameTimer() - modelTimer) > 10000 then break end
                Wait(0)
            end
        ...
        -- ==========================================
        -- MLO / STREAMING FIX (NO HANGS)
        -- ==========================================
        local isInteriorSpawn = false

        -- Force the engine to stream the target area (won't hang like NewLoadSceneStart)
        if SetFocusPosAndVel then
            SetFocusPosAndVel(spawn.x, spawn.y, spawn.z, 0.0, 0.0, 0.0)
        end

        -- Request collision early
        RequestCollisionAtCoord(spawn.x, spawn.y, spawn.z)

        -- Check if we're spawning inside an interior / MLO (with 5s timeout)
        local interior = GetInteriorAtCoords(spawn.x, spawn.y, spawn.z)
        ...
        -- COLLISION & SPAWN
        -- ==========================================
        -- Set coords to trigger area loading
        SetEntityCoordsNoOffset(ped, spawn.x, spawn.y, spawn.z, false, false, false, true)
        NetworkResurrectLocalPlayer(spawn.x, spawn.y, spawn.z, spawn.heading, true, true, false)

        -- Re-apply freeze in case resurrection reset entity flags
        freezePlayer(PlayerId(), true)

        -- Wait for collision to load around entity (8s hard timeout)
        local time = GetGameTimer()
        while not HasCollisionLoadedAroundEntity(ped) and (GetGameTimer() - time) < 8000 do
            RequestCollisionAtCoord(spawn.x, spawn.y, spawn.z)
            Citizen.Wait(0)
        end

        -- Fallback: if collision still isn't loaded, try to snap to ground Z so they don't fall
        if not HasCollisionLoadedAroundEntity(ped) then
            ...
            if foundGround then
                SetEntityCoordsNoOffset(ped, spawn.x, spawn.y, groundZ + 1.0, false, false, false, true)
                NetworkResurrectLocalPlayer(spawn.x, spawn.y, groundZ + 1.0, spawn.heading, true, true, false)
            end
        end

        -- Extra buffer for MLOs so geometry settles before unfreeze
        if isInteriorSpawn then
            Citizen.Wait(500)
        end

        -- Clear forced focus so we don't break other streaming
        if ClearFocus then
            ClearFocus()
        end
        ...
        autoSpawnEnabled = false -- disable auto-spawn after first spawn
        spawnLock = false
    end)
end
            ...
            if autoSpawnEnabled then
                if NetworkIsPlayerActive(PlayerId()) then
                    if (diedAt and (math.abs(GetTimeDifference(GetGameTimer(), diedAt)) > 2000)) or respawnForced then
                        ...
                        autoSpawnEnabled = false -- disable auto-spawn after respawn
                        respawnForced = false
                    end
                end
            end
...
AddEventHandler("playerSpawned", function()
    if InCombatMode then
        TriggerEvent("pvp-weapons:giveLoadout", CurrentMode)
    end
end)
